...
class ManeuverConfig(BaseModel):
    ...
    width_m: float = Field(default=100.0, gt=0, description="Coverage area width in metres (cross-track)")
    height_m: float = Field(default=100.0, gt=0, description="Coverage area height in metres (along-track)")
    sweep_spacing_m: float = Field(default=30.0, ge=30, description="Distance between adjacent sweep strips in metres")

    # per-POI altitude override (None = use global FlightConfig value)
    poi_min_agl_m: float | None = Field(default=None, gt=0, description="POI-specific minimum AGL in metres (overrides global)")
    poi_max_agl_m: float | None = Field(default=None, gt=0, description="POI-specific maximum AGL in metres (overrides global)")
...
class POIConfig(BaseModel):
    name: str | None = Field(default=None, description="Optional human-readable POI name for mission logs")
    point: PointLatLon
...
class FlightConfig(BaseModel):
    ...
    max_surface_radius_m: float = Field(
        default=config.DEFAULT_MAX_SURFACE_RADIUS_M, ge=0,
    ...
    smart_route_corridor_m: float = Field(
        default=config.SMART_ROUTE_CORRIDOR_M, gt=0,
    ...
    max_slope_ratio: float | None = Field(
        default=config.DEFAULT_MAX_SLOPE_RATIO, gt=0.0,
...
class RouteRequest(BaseModel):
    ...
    name: str = Field(default="", description="Optional mission name — used in output filenames and log")
    notes: str = Field(default="", description="Optional free-text notes included in the mission log")
    start: PointLatLon
...
class ViolationInfo(BaseModel):
    point_index: int
    kind: Literal["vertical", "horizontal", "surface_warning", "terrain_band", "slope", "camera_range"]
...
class PlanMeta(BaseModel):
    ...
    poi_scan_good_pct: float | None = None  # % of POI scan points within product spec; None when no POIs
...
class SaveMissionRequest(BaseModel):
    ...
    preset_name: str | None = Field(default=None, description="Name of the drone preset active when the mission was last saved")
...
class AppSettings(BaseModel):
    ...
    default_min_agl_m: float = Field(default=config.DEFAULT_MIN_AGL_M, gt=0,
        description="Default minimum AGL for new missions (metres)")
    default_max_agl_m: float = Field(default=config.DEFAULT_MAX_AGL_M, gt=0,
        description="Default maximum AGL for new missions (metres)")
    default_spacing_m: float = Field(default=config.DEFAULT_SPACING_M, gt=0,
        description="Default route point spacing for new missions (metres)")
    # Safety & warning thresholds
    battery_warning_pct: float = Field(default=config.BATTERY_WARNING_PCT, ge=0, le=200,
        description="Budget % above which an amber battery warning is shown")
    battery_error_pct: float = Field(default=config.BATTERY_ERROR_PCT, ge=0, le=200,
