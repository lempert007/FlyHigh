...
def _split_impossible_legs(
    ...
    for i in range(1, len(points)):
        new_pts, new_acts = _try_split(
            points[i - 1], points[i], actions[i],
            poi_zones, global_band, terrain, spacing, depth=0,
        )
...
def _try_split(
    ...
    left_pts, left_acts = _try_split(
        start, mid, "terrain_split",
        poi_zones, global_band, terrain, spacing, depth + 1,
    )
...
def _insert_ramp_pins(
    ...
    while i < len(result_points) and len(result_points) < safety_limit:
        ...
        for s in profile.samples:
            ...
            drone_alt = result_altitudes[i - 1] + t * (result_altitudes[i] - result_altitudes[i - 1])
...
def plan_altitude_profile(
    ...
    if params.takeoff_alt_msl is not None:
        ...
        if math.isnan(start_elev):
            logger.warning(
                ...
                waypoints_2d[0].lat, waypoints_2d[0].lon,
            )
            start_elev = 0.0
        start_alt = start_elev + global_band.min_agl_m
        logger.info("Start altitude auto-derived: %.1f m MSL (terrain %.1f + min_agl %.1f)",
                    start_alt, start_elev, global_band.min_agl_m)
    ...
    floor_spacing = min(params.spacing_m, _config.FLAT_LEG_SAMPLE_SPACING_M, terrain_dtm.resolution_m)
    ...
    for i in range(1, len(waypoints_2d)):
        ...
        crossings = find_zone_crossings(
            leg_start, leg_end, poi_zones, global_band,
    ...
    expanded_points, expanded_actions = _split_impossible_legs(
        expanded_points, expanded_actions,
        poi_zones, global_band, terrain_dtm, floor_spacing,
    )
    ...
    if not leg_floors:
        key_wps = [Waypoint3D(lat=expanded_points[0].lat, lon=expanded_points[0].lon,
                              alt_msl=start_alt, action=expanded_actions[0])]
    ...
    # ── Step 8: Ramp terrain pins ─────────────────────────────────────────────
    _insert_ramp_pins(
        result_points, result_altitudes, result_actions,
        terrain_dtm, poi_zones, global_band, floor_spacing,
    )
    ...
    logger.info(
        ...
        len(result_points), len(violations),
    )
