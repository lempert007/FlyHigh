...
function toLat(p: PointLike): number { return Array.isArray(p) ? p[0] : p.lat; }
function toLon(p: PointLike): number { return Array.isArray(p) ? p[1] : p.lon; }
...
export function getBatteryColor(pct: number | null | undefined, warnPct = 90, errorPct = 100): string {
