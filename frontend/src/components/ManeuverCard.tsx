...
import {
  Box, Collapse, Grid, IconButton, InputAdornment,
...
export default function ManeuverCard({
  index, poi, onChange, onRemove,
  onActivatePlace, onActivatePolygonDraw, onDragStart, onDrop,
}: ManeuverCardProps) {
  ...
  const updateName = useCallback((val: string) =>
    onChange(index, { ...poi, name: val }), [poi, onChange, index]);

  const updatePoint = useCallback((field: "lat" | "lon", val: string) =>
    onChange(index, { ...poi, point: { ...poi.point, [field]: parseFloat(val) || 0 } }),
    [poi, onChange, index]);

  const updateManeuver = useCallback((field: string, val: string | number | null) =>
    onChange(index, {
      ...poi,
      maneuver: {
        ...poi.maneuver,
        [field]: val === null ? null : typeof val === "string" ? val : parseFloat(String(val)) || 0,
      },
    }), [poi, onChange, index]);
  ...
  const mAdornment = <InputAdornment position="end"><Typography variant="caption" color="text.disabled">m</Typography></InputAdornment>;
    ...
    >
      ...
      <Box sx={{ px: 1.5, pb: 1.5 }}>
        ...
        <Stack direction="row" alignItems="center" spacing={1} mb={1.5}>
          <TextField
            size="small" fullWidth label="Lat" type="number"
            value={point.lat}
            onChange={(e) => updatePoint("lat", e.target.value)}
            inputProps={{ step: 0.0001 }}
          />
          <TextField
            size="small" fullWidth label="Lon" type="number"
            value={point.lon}
            onChange={(e) => updatePoint("lon", e.target.value)}
            inputProps={{ step: 0.0001 }}
          />
          <Tooltip title="Place on map">
            <IconButton size="small" onClick={() => onActivatePlace(index)}
        ...
        <Stack direction="row" alignItems="center" justifyContent="space-between" mb={1.5}>
          <ToggleButtonGroup
            ...
            value={maneuver.type}
            onChange={(_, v) => { if (v) updateManeuver("type", v); }}
            sx={{
          ...
          <Tooltip title={maneuver.polygon
            ? `${maneuver.polygon.length}-vertex polygon — click to redraw`
            : "Draw area on map"}>
            <IconButton size="small" onClick={() => onActivatePolygonDraw?.(index)}
              sx={{ color: maneuver.polygon ? "primary.main" : "text.disabled", "&:hover": { color: "primary.main" } }}>
        ...
        {!maneuver.polygon ? (
          <Stack direction="row" alignItems="center" spacing={1}>
            <TextField
              size="small" type="number" label="Width"
              ...
              sx={{ flex: 1 }}
            />
            <Typography color="text.disabled" sx={{ pb: 0.25 }}>×</Typography>
            <TextField
              size="small" type="number" label="Height"
            ...
            />
            <TextField
              size="small" type="number" label="Sweep"
          ...
          <Stack direction="row" alignItems="center" spacing={1}>
            <TextField
              size="small" fullWidth type="number" label="Sweep"
              value={maneuver.sweep_spacing_m}
              onChange={(e) => updateManeuver("sweep_spacing_m", e.target.value)}
              inputProps={{ step: 1, min: 30 }}
              InputProps={{ endAdornment: mAdornment }}
            ...
            <Typography
              ...
              sx={{ color: "text.disabled", cursor: "pointer", "&:hover": { color: "error.main" }, whiteSpace: "nowrap" }}
        ...
        >
          <Typography variant="caption" sx={{ color: hasAglOverride ? "primary.main" : "text.disabled" }}>
            Height override
          </Typography>
          <ExpandMoreIcon sx={{
            fontSize: 14,
        ...
        <Collapse in={aglOpen}>
          <Grid container spacing={1} sx={{ mt: 0.5 }}>
            <Grid item xs={6}>
              <TextField size="small" fullWidth type="number" label="Min AGL (m)"
                value={maneuver.poi_min_agl_m ?? ""}
                onChange={(e) => updateManeuver("poi_min_agl_m", e.target.value === "" ? null : parseFloat(e.target.value))}
                inputProps={{ step: 5, min: 1 }} placeholder="global"
              />
            </Grid>
            <Grid item xs={6}>
              <TextField size="small" fullWidth type="number" label="Max AGL (m)"
                value={maneuver.poi_max_agl_m ?? ""}
                onChange={(e) => updateManeuver("poi_max_agl_m", e.target.value === "" ? null : parseFloat(e.target.value))}
                inputProps={{ step: 5, min: 1 }} placeholder="global"
              />
            </Grid>
          </Grid>
