  ...
  return (
    <>
      ...
      >
        ...
        >
          <Tooltip
            title={rulerMode ? "Exit measure mode (Esc)" : "Measure distances"}
            placement="right"
          >
            <Box
        ...
        >
          <Tooltip
            title={radiusMode ? "Exit radius mode (Esc)" : "Place radius circle"}
            placement="right"
          >
            <Box
            ...
            {elevationOverlays.length > 1 && (
              <Tooltip title={allVisible ? "Hide all" : "Show all"}>
                <Box
                  onClick={() => {
                    if (allVisible) {
                      elevationOverlays.forEach((o) => {
                        if (visibleOverlays.has(o.filename)) onToggleOverlay(o.filename);
                      });
                    } else {
                      elevationOverlays.forEach((o) => {
                        if (!visibleOverlays.has(o.filename)) onToggleOverlay(o.filename);
          ...
          <Box sx={{ px: 1, py: 0.5 }}>
            {elevationOverlays.map((ov) => {
              ...
              return (
                <Tooltip
                  key={ov.filename}
                  title={`${ov.filename} — click to ${visible ? "hide" : "show"}`}
                  placement="right"
                >
                  <Box
                  ...
                  >
                    ...
                    <Box
                      sx={{
                        ...
                        lineHeight: 1,
                        flexShrink: 0,
                        color: visible ? typeColor : "#30363d",
                      }}
                    >
                      {ov.type}
                    </Box>
