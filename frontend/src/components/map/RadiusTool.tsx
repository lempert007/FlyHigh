...
export function RadiusLayer({ circles, pendingCenter }: RadiusLayerProps) {
  return (
    <>
      {circles.map((c) => (
        <React.Fragment key={c.id}>
          ...
          <Marker
            ...
            icon={makeDistanceIcon(
              `r = ${c.radiusM >= 1000 ? (c.radiusM / 1000).toFixed(2) + " km" : c.radiusM + " m"}`
            )}
            interactive={false}
          />
        </React.Fragment>
      ...
      {pendingCenter && (
        <Marker
          position={[pendingCenter.lat, pendingCenter.lon]}
          icon={makeColorIcon("#ce93d8", "R")}
        />
      )}
    </>
  );
