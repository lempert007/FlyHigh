...
export type DragState =
  ...
  | {
      ...
      initialAltA: number; initialAltB: number;
      startSvgY: number; hasMoved: boolean;
      distA: number; distB: number;
      totalDist: number;
      altMin: number; altRange: number;
      isDraggable: boolean;
    };
...
export function AltitudeChart({
  origAlts, terrain, cumDists, reconAlt, validation,
  nodes, minBand, maxBand, maxAgl,
  dragState, hoveredSegmentKey, hoveredNodeId,
  onHandlePointerDown, onSegmentPointerDown,
  onPointerMove, onPointerUp,
  onNodeDblClick, onSegmentHover, onHandleHover,
}: ChartProps) {
  ...
  const maxBandPtsRev = terrain
    .map((_t, i) => { const ri = terrain.length - 1 - i; return `${cx(cumDists[ri])},${cy(terrain[ri] + maxBand[ri])}`; })
    .join(" ");
  ...
  if (reconAlt.length > 1) {
    ...
    let segColor = validation[0] === "below" ? C_BELOW : validation[0] === "above" ? C_ABOVE : C_PROFILE;
    for (let i = 1; i < reconAlt.length; i++) {
      const c = validation[i] === "below" ? C_BELOW : validation[i] === "above" ? C_ABOVE : C_PROFILE;
      if (c !== segColor) {
  ...
  const poiBlocks = nodes
    .filter((n) => n.type === "poi" && n.poi_start_dist_m !== undefined && n.poi_end_dist_m !== undefined)
    .map((n) => {
  ...
  for (let i = 0; i < handles.length - 1; i++) {
    ...
    const b = handles[i + 1];
    const isDraggable =
      a.type !== "start" && a.type !== "land" &&
      b.type !== "start" && b.type !== "land";
    segmentBars.push({
      key: `${a.id}__${b.id}`,
      idA: a.id, idB: b.id,
      x1: a.svgX, y1: a.svgY,
      x2: b.svgX, y2: b.svgY,
      distA: a.dist_m, distB: b.dist_m,
      isDraggable,
    });
  }
  ...
  // ── Cursor for SVG root ───────────────────────────────────────────────────
  const svgCursor =
    dragState?.type === "handle"  ? "grabbing" :
    dragState?.type === "segment" ? "ns-resize" :
    "default";
  ...
  return (
    ...
    >
      ...
      <polyline points={minBandPts} fill="none" stroke={C_AGL_LINE} strokeWidth={1} strokeDasharray="4 3" />
      <polyline points={maxBandPts} fill="none" stroke={C_AGL_LINE} strokeWidth={1} strokeDasharray="4 3" />

      {/* 3. POI block backgrounds */}
      {poiBlocks.map((b) => (
        <rect key={b.id} x={b.x} y={PT} width={b.w} height={CHART_H}
          fill={C_POI_BG} stroke={C_POI_LINE} strokeWidth={1} strokeDasharray="4 2" />
      ))}
      ...
      {segmentBars.map((seg) => {
        ...
        const isDragging =
          ...
          dragState.idA === seg.idA && dragState.idB === seg.idB;
      ...
      {segments.map((s, i) => (
        <polyline key={i} points={s.pts} fill="none" stroke={s.color} strokeWidth={2} strokeLinejoin="round" />
      ))}

      {/* 6. Axes */}
      <line x1={PL} y1={PT} x2={PL} y2={PT + CHART_H} stroke={C_AXIS} strokeWidth={1} />
      <line x1={PL} y1={PT + CHART_H} x2={PL + CHART_W} y2={PT + CHART_H} stroke={C_AXIS} strokeWidth={1} />

      {/* Y-axis ticks */}
      {yTicks.map(({ a, y }) => (
        <g key={a}>
          <line x1={PL - 4} y1={y} x2={PL} y2={y} stroke={C_AXIS} strokeWidth={1} />
          <text x={PL - 6} y={y + 4} textAnchor="end" fontSize={9} fill={C_LABEL}>{a}</text>
        </g>
      ))}
      <text x={10} y={PT + CHART_H / 2} textAnchor="middle" fontSize={9} fill={C_LABEL}
        transform={`rotate(-90,10,${PT + CHART_H / 2})`}>m MSL</text>

      {/* X-axis ticks */}
      {xTicks.map(({ x, label }) => (
        <g key={label}>
          <line x1={x} y1={PT + CHART_H} x2={x} y2={PT + CHART_H + 4} stroke={C_AXIS} strokeWidth={1} />
          <text x={x} y={PT + CHART_H + 14} textAnchor="middle" fontSize={9} fill={C_LABEL}>{label}</text>
        </g>
      ))}
      ...
      {handles.map((h) => {
        ...
        const fill = isIns ? C_HANDLE_INS : h.type === "land" ? C_LAND : isPoi ? "#00bcd4" : C_HANDLE;
        ...
        return (
          <g key={h.id}>
            {isHovered && !isHandleDragging && (
              <circle cx={h.svgX} cy={h.svgY} r={r + 4}
            ...
            <circle
              cx={h.svgX} cy={h.svgY} r={r}
              fill={fill} stroke="#0d1117" strokeWidth={1.5}
            ...
            {showLabel && (
              <text
                x={h.svgX} y={h.svgY - r - 6}
                textAnchor="middle" fontSize={9} fill={fill} fontWeight="bold"
