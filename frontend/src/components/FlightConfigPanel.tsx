...
const NUM_FIELDS: NumField[] = [
  {
    key: "min_agl_m",
    label: "Min flight height",
    unit: "m",
    min: 1,
    help: "Lowest the drone may fly above the ground (global default)",
  },
  {
    key: "max_agl_m",
    ...
    unit: "m",
    min: 1,
    help: "Highest allowed AGL — overridable per POI",
  },
  {
    key: "point_radius_m",
    label: "Safety radius",
    unit: "m",
    min: 0,
    help: "Hard obstacle-avoidance bubble — HARD violation if anything within this radius exceeds drone altitude. Typically larger than camera range.",
  },
  {
    key: "max_surface_radius_m",
    label: "Camera range",
    unit: "m",
    min: 0,
    help: "HARD check: drone must not exceed max AGL above the lowest surface within this radius. Typically smaller than safety radius.",
  },
  {
    key: "min_altitude_step_m",
    label: "Min altitude step",
    unit: "m",
    min: 0,
    help: "Ignore altitude drops smaller than this value — fly flat at the higher altitude instead. 0 = tightest terrain following.",
  },
  {
    key: "cruise_speed_ms",
    label: "Cruise speed",
    unit: "m/s",
    min: 0.1,
    help: "Horizontal speed during transit",
  },
  {
    key: "climb_rate_ms",
...
export default function FlightConfigPanel({
  ...
  const showTerrainToggles =
    availableTerrainTypes.includes("DSM") && availableTerrainTypes.includes("DTM");
  ...
  const activePreset = useMemo(() => {
    for (const preset of presets) {
      if (Object.entries(preset.config).every(([k, v]) => config[k as keyof FlightConfig] === v))
        return preset.label;
    }
    return "Custom";
  }, [config, presets]);
  ...
  function renderNumField({ key, label, unit, help, min }: NumField) {
    const terrainKey =
      key === "point_radius_m"
        ? ("safety_radius_terrain" as const)
        : key === "max_surface_radius_m"
          ? ("camera_range_terrain" as const)
          : null;
  ...
  return (
    <Paper elevation={2} sx={{ p: 2 }} data-tutorial="flight-config">
          ...
          <Grid container spacing={1}>
            ...
            <Grid item xs={6}>
              {(() => {
                ...
                const activeDenom =
                  config.max_slope_ratio !== null ? Math.round(1 / config.max_slope_ratio) : null;
                const deg = (
                  (Math.atan(config.max_slope_ratio ?? autoRatio) * 180) /
                  Math.PI
                ).toFixed(1);
                return (
                  <Tooltip
                    title="Max ramp steepness — 1 m up per X m forward. Blank = auto from climb rate ÷ speed."
                    placement="top"
                  >
                    <TextField
                      size="small"
                      ...
                      onChange={(e) => {
                        if (e.target.value === "") {
                          onChange({ ...config, max_slope_ratio: null });
                          return;
                        }
      ...
      <Accordion disableGutters>
        ...
        <AccordionDetails>
          <Tooltip
            title="Reorder POIs using a greedy nearest-neighbor algorithm to minimise total transit distance. Has no effect with fewer than 3 POIs."
            placement="top"
          >
            <FormControlLabel
              control={
                <Checkbox
                  size="small"
                  checked={!!config.optimize_poi_order}
                  onChange={handleBool("optimize_poi_order")}
                />
              }
              label={
                <Typography variant="body2">Optimize visit order (nearest-neighbor)</Typography>
              }
            />
          </Tooltip>
          <Tooltip
            title="Shift transit legs sideways to seek flatter terrain before altitude optimisation. Reduces unnecessary height changes at the cost of slightly longer planning time."
            placement="top"
          >
            <FormControlLabel
              control={
                <Checkbox
                  size="small"
                  checked={!!config.smart_route}
                  onChange={handleBool("smart_route")}
                />
              }
              label={
                <Typography variant="body2">Smart Route — lateral path optimisation</Typography>
              }
            />
          </Tooltip>
          {config.smart_route && (
            <Tooltip
              title="Maximum sideways offset the optimizer may try per transit segment."
              placement="top"
            >
              <TextField
                size="small"
                fullWidth
                label="Max lateral offset (m)"
                type="number"
