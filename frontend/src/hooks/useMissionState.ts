...
export function useMissionState(): MissionStateReturn {
  const {
    ...
    canRedo,
    reset: resetHistory,
  } = useUndoRedo<MissionSnapshot>(INITIAL_SNAPSHOT);
  ...
  const setWaypoints = useCallback<React.Dispatch<React.SetStateAction<Waypoint[]>>>(
    (updater) => {
  ...
  const setPois = useCallback<React.Dispatch<React.SetStateAction<Poi[]>>>(
    (updater) => {
      const cur = snapRef.current;
      ...
      setInteraction((prev) => {
        ...
        if (prev.mode === "poi") {
          if (prev.poiIndex !== null) {
            setPois((ps) =>
              ps.map((p, i) => (i === prev.poiIndex ? { ...p, point: { lat, lon } } : p))
            );
          } else {
  ...
  const handlePoiRemove = useCallback(
    (index: number) => {
  ...
  const handlePoiDrop = useCallback(
    (toIndex: number) => {
  ...
  const handlePolygonVertex = useCallback(({ lat, lon }: LatLon) => {
    setInteraction((prev) => ({
      ...prev,
      polygonVertices: [...prev.polygonVertices, { lat, lon }],
    }));
  }, []);
  ...
  const handlePolygonVertexDrag = useCallback(
    (poiIdx: number, vi: number, { lat, lon }: LatLon) => {
      setPois((prev) =>
        prev.map((p, i) =>
          i === poiIdx
            ? {
                ...
                maneuver: {
                  ...p.maneuver,
                  polygon: p.maneuver.polygon?.map((v, j) => (j === vi ? { lat, lon } : v)),
                },
              }
            : p
        )
  ...
  const handlePolygonClose = useCallback(() => {
    setInteraction((prev) => {
      if (prev.mode === "polygon" && prev.poiIndex !== null && prev.polygonVertices.length >= 3) {
        ...
        setPois((ps) =>
          ps.map((p, i) =>
            i === poiIndex ? { ...p, maneuver: { ...p.maneuver, polygon: verts } } : p
          )
        );
      }
      return INTERACTION_RESET;
  ...
  return {
    ...
    handlePoiDrag,
    handlePoiRemove,
    handlePoiActivatePlace,
    handlePoiDragStart,
    handlePoiDrop,
    handleAddPoi,
    handleActivatePolygonDraw,
    handlePolygonVertex,
    handlePolygonVertexDragInProgress,
