...
export const PLAN_STEPS: PlanStep[] = [
  // Terrain
  { phase: "terrain",   phaseLabel: "Loading Terrain",       msg: "Opening raster tiles from session store" },
  { phase: "terrain",   phaseLabel: "Loading Terrain",       msg: "Reprojecting CRS → UTM coordinate frame" },
  ...
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Sampling terrain profile along full route" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Computing AGL floor/ceiling at each sample" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Backward floor propagation pass" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Forward floor propagation pass" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Forward ceiling propagation pass" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Backward ceiling propagation pass" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Selecting cruise altitudes (fewest changes)" },
  { phase: "altitude",  phaseLabel: "Altitude Optimization", msg: "Inserting ramp waypoints along descents" },
  // Safety
...
type OnSuccessCallback = (meta: PlanMeta | null, blob: Blob, routePoints: LatLon[], routeAglProfile: number[] | null) => void;
...
export function usePlanRoute(inputs: PlanInputs, onSuccess: OnSuccessCallback): UsePlanRouteReturn {
  ...
  const handlePlan = useCallback(async () => {
    const { sessionId, start, pois, waypoints, flightConfig, missionName, missionNotes, takeoffMode, takeoffAltM } = inputs;
    ...
    try {
      const request = buildRouteRequest({
        start, waypoints, pois, config: flightConfig,
        name: missionName, notes: missionNotes,
      ...
      try {
        ...
        if (wpFile) {
          const wps = JSON.parse(await wpFile.async("string")) as Array<{ lat: number; lon: number }>;
